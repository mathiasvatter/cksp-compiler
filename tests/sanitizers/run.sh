...
#
# Compiles every .ksp / .cksp file under tests/ with an AddressSanitizer build and fails on any
# sanitizer report. The bugs this catches do not show up as a failed compile: the AST is built from
...
#
# Without an argument an AddressSanitizer build is configured and built in cmake-build-asan/.
...
if [[ -z "$EXEC" ]]; then
  ...
  cmake -S "$BASE_DIR" -B "$BASE_DIR/cmake-build-asan" \
    ...
    -DCMAKE_EXE_LINKER_FLAGS="-fsanitize=address" >/dev/null || exit $?
