...
class ConstExprPropagation final : public ASTOptimizations {
	...
	void add_constant_expression(NodeDataStructure& data, NodeAST* value) {
		// if mutable, try to propagate the value
		if(data.data_type == DataType::Mutable and data.get_node_type() == NodeType::Variable) {
			if(value->is_constant(true)) {
				m_constant_expressions[get_hash_value(data)] = value->clone();
			}
		}
	}

	void add_constant_expression(NodeReference& ref, NodeAST* value) {
		if(value->is_constant(true)) {
			m_constant_expressions[get_hash_value(ref)] = value->clone();
		}
	}
...
public:
	...
	NodeAST* visit(NodeSingleDeclaration& node) override {
		...
		if(node.value) {
			...
			// if mutable, try to propagate the value
			add_constant_expression(*node.variable, node.value.get());
		}
		return &node;
	}
	...
	NodeAST* visit(NodeSingleAssignment& node) override {
		...
		if(node.l_value->data_type == DataType::Mutable) {
			add_constant_expression(*node.l_value, node.r_value.get());
		}
