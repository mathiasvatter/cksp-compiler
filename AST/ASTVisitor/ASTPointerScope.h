...
//		}
	...
	std::unique_ptr<NodeBlock> retain_ptr_ptr(NodePointerRef* l_ref, NodePointerRef* r_ref) {
	...
	std::unique_ptr<NodeBlock> retain_arr_init(NodeArrayRef* l_ref, NodeInitializerList* r_ref) {
		...
		if(r_ref->size() == 1) {
			...
			arr_ref->set_index(std::make_unique<NodeInt>(r_ref->size()-1, l_ref->tok));
			retain->add_as_single_retain(
				clone_as<NodeReference>(arr_ref.get()),
	...
	std::unique_ptr<NodeBlock> add_retain(NodeSingleDeclaration& declaration) {
		...
		if(l_pointer_ref and r_pointer_ref) {
			return retain_ptr_ptr(l_pointer_ref, r_pointer_ref);
		}
		...
		if(l_array_ref and r_initializer_list) {
			return retain_arr_init(l_array_ref, r_initializer_list);
		}

		auto r_array_ref = cast_node<NodeArrayRef>(value.get());
		if(l_array_ref and r_array_ref) {
			return retain_arr_arr(l_array_ref, r_array_ref);
		}
