...
//			generate_iterators(array_to_copy);
	...
	std::unique_ptr<NodeFunctionDefinition> generate_function_def() {
		...
		body->add_stmt(std::make_unique<NodeStatement>(std::move(assignment), Token()));
...
private:
			...
			for (auto &idx : node_nd_array_ref->indexes->params) {
				if (idx->get_node_type() == NodeType::Wildcard) {
					auto node_upper_bound = std::make_unique<NodeVariableRef>(
						"original.SIZE_D" + std::to_string(count), Token());
	...
	void generate_iterators(NodeReference* array_ref) {
		...
		if(array_ref->get_node_type() == NodeType::ArrayRef) {
			auto node_iterator = std::make_unique<NodeVariable>(std::nullopt, "_iter"+std::to_string(iter_size), TypeRegistry::Integer, DataType::Mutable, Token());
	...
	void generate_dims_params(NodeReference* array_ref) {
		...
		// if normal array -> only one _iter1
		if(array_ref->get_node_type() == NodeType::ArrayRef) {
			auto node_dim = std::make_unique<NodeVariable>(std::nullopt, "dim"+std::to_string(dims_size), TypeRegistry::Integer, DataType::Mutable, Token());
		...
		if(array_ref->get_node_type() == NodeType::NDArrayRef) {
			...
			for (auto &idx : node_nd_array_ref->indexes->params) {
				if (idx->get_node_type() != NodeType::Wildcard) {
					auto node_dim = std::make_unique<NodeVariable>(std::nullopt, "dim" + std::to_string(count), TypeRegistry::Integer, DataType::Mutable, Token());
					dim_vars_to_copy.push_back(std::move(node_dim));
	...
	std::unique_ptr<NodeReference> remove_indexes(NodeReference* array_ref) {
		if(array_ref->get_node_type() == NodeType::ArrayRef) {
			auto node_array_ref = clone_as<NodeArrayRef>(array_ref);
	...
	std::string generate_array_string(NodeReference* ref) {
		std::string name;
		if(array->get_node_type() == NodeType::ArrayRef) {
			name += "array";
		}
		if(array->get_node_type() == NodeType::NDArrayRef) {
			name += "ndarray";
			auto ndarray = static_cast<NodeNDArrayRef*>(array);
			// add dimension size
