...
namespace CrashReporter {
	namespace detail {
		...
		std::string shell_join_command_line(const int argc, char* argv[]) {
			std::string result;
		...
		std::string configured_crash_log_path() {
...
#if defined(__APPLE__) || defined(__linux__)
		void write_text(const int fd, const char* text) {
			::write(fd, text, std::strlen(text));
		}

		void write_text(const int fd, const std::string& text) {
		...
		void write_crash_report(const int fd, const int signal) {
		...
		void crash_handler(const int signal) {
			const auto& crash_state = state();
		...
		bool& symbol_handler_ready() {
			static bool ready = false;
			return ready;
		}

		void initialize_symbol_handler() {
		...
		ResolvedAddress resolve_address(const void* address) {
		...
		void write_address(const HANDLE output, const char* prefix, const void* address) {
		...
		void write_stacktrace(const HANDLE output) {
			void* frames[64]{};
		...
		volatile LONG& crash_in_progress() {
	...
	void install(const char* mode, const int argc, char* argv[]) {
