...
Result<std::string> PathHandler::check_valid_path(const std::string &path) {
	...
	auto error = CompileError(ErrorType::FileError,
						 "Found incorrect path.", m_current_token.line, "valid path", absPath.string(), m_current_file);
...
Result<std::string> PathHandler::generate_output_file(const std::string &absolute_path) {
	...
	auto error = CompileError(ErrorType::FileError,"", m_current_token.line, "valid path", path.string(), m_current_file);
	if (!path.is_absolute()) {
		error.m_message = "Given path is not absolute.";
		error.m_expected = "absolute path";
		return Result<std::string>(error);
	}

	// Überprüfen, ob der Pfad eine .txt-Dateiendung hat
	if (path.extension() != ".txt") {
		error.m_message = "Given path does not lead to a .txt file.";
		error.m_expected = "*.txt file";
		return Result<std::string>(error);
	}

	// Überprüfen, ob der übergeordnete Ordner existiert
	if (!std::filesystem::exists(path.parent_path())) {
		error.m_message = "The parent folder does not exist.";
		error.m_expected = path.parent_path().string();
		return Result<std::string>(error);
	}
	...
	if (!outfile) {
		error.m_message = "Could not open file.";
		return Result<std::string>(error);
	}
	outfile.close();
...
Result<std::string> PathHandler::resolve_path(const std::string &import_path) {
	...
	auto error = CompileError(ErrorType::FileError,
	"Found incorrect path.", m_current_token.line, "valid path", rel.string(), m_current_file);
	...
	if (is_directory(absPath) and std::filesystem::exists(absPath)) {
		...
		if(new_absPath.is_error()) {
			error.m_message = "Could not resolve paths.";
			error.m_got = current_base.string() + ", " + rel.string();
			return Result<std::string>(error);
		}
