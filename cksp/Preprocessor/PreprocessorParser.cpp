...
Result<std::unique_ptr<PreNodeProgram>> PreprocessorParser::parse_program(PreNodeAST *parent) {
    ...
    const auto token_count = m_tokens.size();
    program->chunk.reserve(token_count);
    define_statements.reserve(token_count / 32 + 1);
    macro_definitions.reserve(token_count / 64 + 1);
    ...
    while(peek().type != token::END_TOKEN) {
        ...
        auto parse_result = parse_main_constructs(parent, m_program->program);
        if (parse_result.is_error()) {
            return Result<std::unique_ptr<PreNodeProgram>>(parse_result.get_error());
        }
    }
...
Result<SuccessTag> PreprocessorParser::parse_main_constructs(PreNodeAST *parent, std::unique_ptr<PreNodeChunk> &chunk) {
    if (is_define_definition()) {
        ...
        if (token_result.is_error())
            return Result<SuccessTag>(token_result.get_error());
        chunk->add_chunk(std::move(token_result.unwrap()));
    }
...
Result<std::unique_ptr<PreNodeAST>> PreprocessorParser::parse_int(PreNodeAST *parent) {
    auto token = consume();
    const auto &value = token.val;
    long long val = 0;
    const auto *begin = value.data();
    const auto *end = begin + value.size();
    const auto parse_res = std::from_chars(begin, end, val, 10);
...
Result<std::unique_ptr<PreNodeAST>> PreprocessorParser::parse_token(PreNodeAST* parent) {
    ...
    const auto curr_type = curr.type;
    const auto start_token = curr;
    ...
    if (is_macro_definition()) {
    ...
    } else if (is_iterate_macro_call(curr)) {
        ...
        stmt = std::move(node_statement);
    } else if (curr_type == token::RESET_CONDITION) {
        auto result_condition_def = parse_reset_condition(node_statement.get());
        if (result_condition_def.is_error())
            return Result<std::unique_ptr<PreNodeAST>>(result_condition_def.get_error());
        node_statement->statement = std::move(result_condition_def.unwrap());
        stmt = std::move(node_statement);
    } else if (curr_type == token::IMPORT) {
        ...
        stmt = std::move(node_statement);
    } else if (curr_type == token::USE_CODE_IF || curr_type == token::USE_CODE_IF_NOT) {
        ...
        stmt = std::move(node_statement);
    } else if (curr_type == token::KEYWORD && curr.val == "import_nckp") {
        ...
        stmt = std::move(node_statement);
    } else if (is_function_call(curr)) {
        auto result_function_call  = parse_function_call(node_statement.get());
        if (result_function_call.is_error())
            return Result<std::unique_ptr<PreNodeAST>>(result_function_call.get_error());
        node_statement->statement = std::move(result_function_call.unwrap());
        stmt = std::move(node_statement);
    } else if(curr_type == token::KEYWORD || curr_type == token::STRING || curr_type == token::DEFAULT || curr_type == token::RETURN) {
        ...
        stmt = std::move(node_statement);
    } else if(curr_type == token::FLOAT || curr_type == token::BINARY || curr_type == token::HEXADECIMAL) {
        ...
        stmt = std::move(node_statement);
    } else if (curr_type == token::START_INC) {
        ...
        stmt = std::move(node_statement);
    } else if (curr_type == token::PRAGMA) {
    ...
    if (peek().type != token::CLOSED_PARENTH) {
        ...
        while (parenth_depth > 0) {
            ...
            if (peek().type == token::OPEN_PARENTH) {
                ...
                if (parenth_depth == 1 && bracket_depth > 0) {
                    auto error = Diagnostic(ErrorType::PreprocessorError, "", "]", peek());
                    error.set_message("Unexpected closing parenthesis in <list> syntax.");
                    error.add_message("Missing closing bracket before ')'.");
                    return Result<std::unique_ptr<PreNodeList>>(error);
                }
                parenth_depth--;
                ...
                if (bracket_depth == 0) {
                    auto error = Diagnostic(ErrorType::PreprocessorError, "", "[", peek());
                    error.set_message("Unexpected closing bracket in <list> syntax.");
                    return Result<std::unique_ptr<PreNodeList>>(error);
                }
                bracket_depth--;
            } else if (peek().type == token::END_TOKEN) {
                return Result<std::unique_ptr<PreNodeList>>(Diagnostic(ErrorType::PreprocessorError,
                                                                         "Unexpected end of file. Missing closing parenthesis.",")", peek()));
                //         } else if (peek().type == token::LINEBRK) {
                // return Result<std::unique_ptr<PreNodeList>>(Diagnostic(ErrorType::SyntaxError,
                //  "Unexpected linebreak. Missing closing parenthesis.",")", peek()));
            }
            if (peek().type == token::COMMA && parenth_depth == 1 && bracket_depth == 0) {
...
Result<std::unique_ptr<PreNodeDefineHeader>> PreprocessorParser::parse_define_header(PreNodeAST *parent) {
    ...
    if(peek().type == token::OPEN_PARENTH) {
        node_define_header->has_parenth = true;
...
Result<std::unique_ptr<PreNodeMacroDefinition>> PreprocessorParser::parse_macro_definition(PreNodeAST* parent) {
    ...
    while(peek().type != token::END_MACRO) {
        ...
        auto parse_result = parse_main_constructs(parent, node_chunk);
        if (parse_result.is_error()) {
            return Result<std::unique_ptr<PreNodeMacroDefinition>>(parse_result.get_error());
        }
    }
...
Result<std::unique_ptr<PreNodeFunctionCall>> PreprocessorParser::parse_function_call(PreNodeAST *parent) {
    auto start_token = peek();
    auto node_function_call = std::make_unique<PreNodeFunctionCall>(start_token, parent);
    auto header = parse_function_header(node_function_call.get());
    if(header.is_error()){
        return Result<std::unique_ptr<PreNodeFunctionCall>>(header.get_error());
    }
    node_function_call->set_header(std::move(header.unwrap()));
    node_function_call->set_range(start_token, peek(-1));
    return Result<std::unique_ptr<PreNodeFunctionCall>>(std::move(node_function_call));
}

Result<std::unique_ptr<PreNodeFunctionHeader>> PreprocessorParser::parse_function_header(PreNodeAST *parent) {
    auto start_token = peek();
    auto node_function_header = std::make_unique<PreNodeFunctionHeader>( start_token, parent);
    auto function_name = parse_keyword(node_function_header.get());
    if(function_name.is_error())
        return Result<std::unique_ptr<PreNodeFunctionHeader>>(function_name.get_error());
    node_function_header->name = std::move(function_name.unwrap());
    auto function_args = std::make_unique<PreNodeList>(peek(), node_function_header.get());
    if (peek().type == token::OPEN_PARENTH) {
        node_function_header->has_parenth = true;
        auto macro_args_result = parse_list(node_function_header.get());
        if (macro_args_result.is_error())
            return Result<std::unique_ptr<PreNodeFunctionHeader>>(macro_args_result.get_error());
        function_args = std::move(macro_args_result.unwrap());
    }
    node_function_header->set_args(std::move(function_args));
    node_function_header->set_range(start_token, peek(-1));
    return Result<std::unique_ptr<PreNodeFunctionHeader>>(std::move(node_function_header));
}
...
Result<std::unique_ptr<PreNodeIncrementer>> PreprocessorParser::parse_incrementer(PreNodeAST *parent) {
    ...
    if(peek().type != token::OPEN_PARENTH) {
        auto error = Diagnostic(ErrorType::PreprocessorError, error_msg, "(", peek());
    ...
    if(list->params.size() != 3) {
        auto error = Diagnostic(ErrorType::PreprocessorError, error_msg, "<name>, <start>, <step>", peek());
        error.set_message("Expected exactly 3 parameters in <START_INC> statement syntax: <name>, <start>, <step>.");
        error.actual = list->get_string();
        return Result<std::unique_ptr<PreNodeIncrementer>>(error);
    }
    // check if <name> argument only consists of one token
    if(list->params[0]->num_chunks() != 1) {
        auto error = Diagnostic(ErrorType::PreprocessorError,error_msg, "<name>", peek());
        error.actual = list->params[0]->get_string();
        error.add_message("Found too many tokens in <START_INC> <name> argument.");
        return Result<std::unique_ptr<PreNodeIncrementer>>(error);
    }
    if (peek().type != token::LINEBRK) {
        auto error = Diagnostic(ErrorType::PreprocessorError, error_msg, "linebreak", peek());
    ...
    while (peek().type != token::END_INC) {
        ...
        while(peek().type != token::LINEBRK) {
            ...
            if(peek().type == token::END_TOKEN) {
                auto error = Diagnostic(ErrorType::PreprocessorError, error_msg, "end", peek());
                error.set_message("Missing <END_INC> statement. Reached the end of the file while parsing an incrementer.");
                return Result<std::unique_ptr<PreNodeIncrementer>>(error);
            }
            auto parse_result = parse_main_constructs(parent, node_chunk);
            if (parse_result.is_error()) {
                return Result<std::unique_ptr<PreNodeIncrementer>>(parse_result.get_error());
            }
...
Result<std::unique_ptr<PreNodeImport>> PreprocessorParser::parse_import(PreNodeAST *parent) {
    ...
    if(path.type != token::STRING) {
        auto error = Diagnostic(ErrorType::FileError, "Not a filepath.","path",path);
    ...
    if(peek().type != token::LINEBRK) {
        auto error = Diagnostic(ErrorType::ParseError, "Incorrect <import> syntax.","linebreak",peek());
...
Result<std::unique_ptr<PreNodeImportNCKP>> PreprocessorParser::parse_import_nckp(PreNodeAST *parent) {
    ...
    if(peek().type != token::STRING) {
        return Result<std::unique_ptr<PreNodeImportNCKP>>(Diagnostic(ErrorType::PreprocessorError,
...
Result<std::unique_ptr<PreNodeSetCondition>> PreprocessorParser::parse_set_condition(PreNodeAST *parent) {
    ...
    consume(); // consume (
    if(peek().type != token::KEYWORD) {
        auto error = Diagnostic(ErrorType::PreprocessorError, error_msg,"<condition-symbol>",peek());
...
Result<std::unique_ptr<PreNodeUseCodeIf>> PreprocessorParser::parse_use_code_if(PreNodeAST *parent) {
    ...
    while (peek().type != token::END_USE_CODE) {
        ...
        auto parse_result = parse_main_constructs(parent, node_chunk);
        if (parse_result.is_error()) {
            return Result<std::unique_ptr<PreNodeUseCodeIf>>(parse_result.get_error());
        }
    }
    auto end_inc = consume(); // consume END_INC
...
bool PreprocessorParser::is_function_call(const Token &tok) const {
    if (tok.type != token::KEYWORD) return false;
    auto function_syntax = tok.type == token::KEYWORD && peek_type(1) == token::OPEN_PARENTH;
    auto macro_syntax = tok.type == token::KEYWORD && (peek_type(1) == token::LINEBRK || peek_type(1) == token::OPEN_PARENTH);
    if (m_pos > 1) {
        macro_syntax = peek_type(-1) == token::LINEBRK && tok.type == token::KEYWORD && (peek_type(1) == token::LINEBRK || peek_type(1) == token::OPEN_PARENTH);
    }
    return function_syntax || macro_syntax;
}
