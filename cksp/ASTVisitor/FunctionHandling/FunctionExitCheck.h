 ...
 *  - Diagnostic: when encountering return AND exit statements
...
public:
	...
	NodeAST* sanitize_exit(NodeFunctionDefinition& def) {
		...
		auto warning = Diagnostic(ErrorType::CompileWarning, "", "", def.tok);
		...
		warning.report(def.diagnostics());
...
private:
	...
	NodeAST* visit(NodeReturn &node) override {
		auto error = Diagnostic(ErrorType::CompileError, "", "", node.tok);
