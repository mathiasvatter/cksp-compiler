...
Result<std::unique_ptr<PreNodeProgram>> PreprocessorParser::parse_program(PreNodeAST *parent) {
    ...
    while(peek(m_tokens).type != END_TOKEN) {
        if (peek(m_tokens).type == DEFINE) {
            ...
            m_define_statements.push_back(std::move(result_define.unwrap()));
...
Result<std::unique_ptr<PreNodeAST>> PreprocessorParser::parse_token(PreNodeAST* parent) {
    ...
    if(is_define_call(peek(m_tokens))) {
        ...
        stmt = std::move(std::move(node_statement));
        ...
        stmt = std::move(std::move(result_keyword.unwrap()));
        ...
        stmt = std::move(result_number.unwrap());
    } else {
...
Result<std::unique_ptr<PreNodeList>> PreprocessorParser::parse_list(PreNodeAST *parent) {
    ...
    if (peek(m_tokens).type != token::CLOSED_PARENTH) {
        ...
        std::vector<std::unique_ptr<PreNodeAST>> arg;
        auto node_chunk = std::make_unique<PreNodeChunk>(std::move(arg), node_list.get());
...
Result<std::unique_ptr<PreNodeDefineStatement>> PreprocessorParser::parse_define_definition(PreNodeAST *parent) {
    ...
    auto define_statement = std::make_unique<PreNodeDefineStatement>(std::move(header), std::move(body), parent);
    ...
    while(peek(m_tokens).type != LINEBRK) {
        ...
        if(peek(m_tokens).type == KEYWORD and define_header_result.unwrap()->name == peek(m_tokens).val) {
