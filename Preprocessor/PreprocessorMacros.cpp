    ...
    while (peek(m_tokens).type != token::END_TOKEN) {
        if(peek(m_tokens).type == token::MACRO) {
            ...
            if(macro_definition.is_error())
                ...
                m_macro_evaluation_stack.push_back(std::move(macro_definition.unwrap()));
        } else {
            consume(m_tokens);
        }
    }
...
//
//
Result<std::string> PreprocessorMacros::parse_macro_definition(std::vector<Token>& tok) {
    consume(tok); //consume "macro"
    if (peek(tok).type != token::KEYWORD) {
        return Result<std::string>(CompileError(ErrorType::SyntaxError,
	 "Missing macro name.",peek(tok).line,"keyword",peek(tok).val, peek(tok).file));
    }
    return Result<std::string>(consume(tok).val);
}
//
